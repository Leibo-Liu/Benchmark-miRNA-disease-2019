...
if __name__ == '__main__': 
    ...
    data = pd.read_table("./normalization_data/Z_score data/"+predicted[0]+".txt",index_col=0)
    ...
    for i in range(1,10):
        ...
        data = pd.read_table("./normalization_data/Z_score data/"+predicted[i-1]+".txt",index_col=0)
        data.columns=["Disease","miRNA","Scort","y_ture","Feature_scort"]
        vs[i-1] = data.drop_duplicates(subset=["Disease","miRNA"])
        
        for j in range(1,len(predictors)):
            if predictors[j] not in predicted:
                data = pd.read_table("./normalization_data/Z_score data/"+predictors[j]+".txt",index_col=0 )
                data.columns =["Disease","miRNA","Scort","y_ture","Feature_scort"] 
                data= data.drop_duplicates(subset=["Disease","miRNA"])
        ...
        with open("./Results/Z_score_data_result.txt","a+") as f:
            f.write(save_str)
            f.close()