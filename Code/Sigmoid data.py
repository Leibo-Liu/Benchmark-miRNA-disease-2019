...
if __name__ == '__main__': 
    ...
    data = pd.read_table("./normalization_data/Sigmoid data/"+predicted[0]+".txt",index_col=0)
    ...
    for i in range(1,10):
        ...
        data = pd.read_table("./normalization_data/Sigmoid data/"+predicted[i-1]+".txt",index_col=0)
        data.columns=["Disease","miRNA","Scort","y_ture","Feature_scort"]
        ...
        for j in range(1,len(predictors)):
            if predictors[j] not in predicted:
                t1=time.time()
                data = pd.read_table("./normalization_data/Sigmoid data/"+predictors[j]+".txt",index_col=0 )
                data.columns =["Disease","miRNA","Scort","y_ture","Feature_scort"] 
                data= data.drop_duplicates(subset=["Disease","miRNA"])
        ...
        with open("./Results/Sigmoid_data_result.txt","a+") as f:
            f.write(save_str)
            f.close()